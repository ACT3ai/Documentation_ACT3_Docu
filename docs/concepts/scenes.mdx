...
- **Time of day** — Morning, afternoon, night, etc., which affects [lighting](/how-to/set-up-lighting)
- **Characters** — The actors present in this scene
- **[Beats](/concepts/beats)** — The key dramatic moments that make up the scene
- **[Shots](/concepts/shots)** — The individual camera angles that will be rendered
...
3. Click **Add Scene** or let scene headings (INT./EXT.) in your imported [script](/features/script-editing) create scenes automatically
...
In team projects, scenes can be locked once approved to prevent changes. See [Collaboration](/features/collaboration) for full details on team workflows.
...
You can generate video previews for an entire scene at once, or render individual shots within a scene. Scene-level rendering sends all shots in sequence to the [render queue](/features/render-queue), producing a continuous clip for review. See [How to manage the render queue](/how-to/manage-render-queue) for tips on scheduling and prioritizing jobs.
...
- Align scenes with [act](/concepts/acts) structure so dramatic pacing flows naturally
- Use scene locking early in collaborative projects to avoid conflicts — see [Lock Approved Elements](/how-to/lock-approved-elements)
